return function()
	...
	local canChat =game:GetService("Chat"):CanUserChatAsync(p.UserId)

	local Player = game.Players.LocalPlayer 
	while Player.Character == nil do wait() end 
	local RbxUtility = _G.LoadLibrary('RbxUtility')
	...
	function Chat:CreateGui()
		self.Gui = WaitForChild(script.Parent.Parent, 'RobloxGui')
		...
		if canChat then--Player.ChatMode == Enum.ChatMode.TextAndMenu then 	
			if Chat:IsTouchDevice() then 
	...
	function Chat:PlayerChatted(...)
		...
		if PlayersService.ClassicChat then 				
			...
			elseif  string.sub(message, 3) == '/sc' then 			
				Chat:UpdateChat(player, message)
			else 
	...
	function Chat:Initialize()		
		Chat:CreateGui() 		
		self.EventListener = tcs.MessageReceived:Connect(function(msg) 	
			-- This event has 4 callback arguments 
			-- Enum.PlayerChatType.All, chatPlayer, message, targetPlayer 
			if msg.TextSource.UserId then
				local plr = game.Players:GetPlayerByUserId(msg.TextSource.UserId)
				Chat:PlayerChatted(Enum.PlayerChatType.All,plr,msg.Text,game.Players.LocalPlayer)
			else
				Chat:PlayerChatted(Enum.PlayerChatType.All,nil,msg.Text,game.Players.LocalPlayer)
