...
return function()
	...
	local currentLoadout = script.Parent:WaitForChild("CurrentLoadout")
	...
	function activateLoadout()
		keyPressCon = game:GetService("UserInputService").InputBegan:Connect(function(key,sank)
			if not sank and tostring(key.KeyCode) and key.KeyCode and key.KeyCode.Name then
				activateGear(a[tostring(key.KeyCode.Name)])
			end
		end)
